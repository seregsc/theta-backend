...
def fetch_prices_with_history(supabase):
    """Carica ultimi prezzi + history per ogni ticker. Ritorna dict {ticker: {price, change_1d, change_7d, change_30d, currency, name}}"""
    # 1. Ultimi prezzi
    ...
    # Calcola variazioni per ogni ticker
    ...
    for ticker, p in prices.items():
        ...
        # variazione 1d (già nella tabella prices)
        change_1d = p.get("change_percent")
        # variazione 7d/30d da history
        change_7d = None
        change_30d = None
        for h in hist:
            h_date = datetime.fromisoformat(h["date"]).date() if isinstance(h["date"], str) else h["date"]
...
def fetch_recent_news(supabase, hours=72, limit=30):
    """Carica le news HIGH degli ultimi N ore."""
...
def compute_crolli(prices_data, top_n=8):
    ...
    for ticker, p in prices_data.items():
        # Considera change_1d + change_7d (più affidabili che change_30d perché abbiamo storico recente)
        ch_1d = p.get("change_1d") or 0
        ch_7d = p.get("change_7d")
        # Filtra: change_1d < -3% O change_7d < -7%
        is_crash_1d = ch_1d <= -3
        is_crash_7d = ch_7d is not None and ch_7d <= -7
        if not (is_crash_1d or is_crash_7d):
            continue
        # Score: drawdown peggiore ha score più alto
        worst = min([x for x in [ch_1d, ch_7d, p.get("change_30d") or 0] if x is not None])
        ...
        reason = f"Calo significativo del prezzo nelle ultime sessioni. Possibile occasione di ingresso se la tesi sull'asset è ancora valida."
        candidates.append({
            "category": "crolli",
            "ticker": ticker,
...
def compute_beneficiari(news_list, prices_data, top_n=6):
    ...
    if not news_list or not ANTHROPIC_API_KEY:
        return []
    # Costruisci sintesi news per Claude
    news_summary = []
    for n in news_list[:15]:
        ...
        news_summary.append(f"- [{sentiment.upper()}] {title}\n  {summary}\n  Ticker citati: {', '.join(tickers) if tickers else 'nessuno'}")
    news_text = "\n\n".join(news_summary)

    # Lista ticker disponibili in Theta (filtrata per non includere quelli che non hanno prezzo)
...
def compute_sottovalutati(prices_data, top_n=6):
    """Asset con flat/leggero ribasso ma fondamentalmente attraenti.
    Per ora usiamo logiche semplici. Quando avremo P/E, target price reali, fingiremo più finezza."""
    candidates = []
    for ticker, p in prices_data.items():
        ch_7d = p.get("change_7d")
        ch_30d = p.get("change_30d")
        # Filtro: nessun crollo brutale, ma drawdown mensile -5% a -15% (asset "dimenticato")
        if ch_30d is None:
            continue
        if not (-15 <= ch_30d <= -5):
            continue
        # Se è in crash brutale, lo classifichiamo come "crolli" non "sottovalutati"
        if (ch_7d or 0) < -10:
            continue
        ...
        candidates.append({
            ...
            "reason": f"L'asset è in flat/ribasso moderato senza crash brutali. Possibile valore se i fondamentali sono solidi.",
            "expected_move": "+5-12%",
            "time_horizon": "medium",
            "risk": "MED",
        })
...
def main():
    ...
    # Cancella tutte le opportunità vecchie (oltre 36 ore = troppo vecchie)
    ...
    for op in all_ops:
        try:
            ...
            # Lookup name from prices table is optional
